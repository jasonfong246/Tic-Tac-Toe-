...
namespace CrawfisSoftware.TicTacToeFramework
{
        ...
        where TCellStateEnum : struct, System.Enum
    {
        ...
        public void ChangeCellAttempt(int cellID, TCellStateEnum newCellState)
        {
            ...
            {
                Console.WriteLine("This block has been chosen already.");
            }
        }
        ...
        {
            TCellStateEnum currentCellValue = gameBoard[cellID];
            if (newCellState.ToString().Equals("X"))
            {
                if (replaceCountX > 0)
                {
                    if (currentCellValue.ToString().Equals("X"))
                    {
                        ChangeCellStrategy(cellID, newCellState, currentCellValue);
                    }
                    else
                    {
                        replaceCountX--;
                        ChangeCellStrategy(cellID, newCellState, currentCellValue);
                        Console.WriteLine("You have " + replaceCountX + " attempts in changing the cell blocks");
                    }
                }
                else
                {
            ...
            else if (newCellState.ToString().Equals("O"))
            {
                if (replaceCountO > 0)
                {
